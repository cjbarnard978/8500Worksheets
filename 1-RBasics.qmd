...
```{r}
paste("I made lunch", "and dinner")
```
I made lunch and dinner

(@) Can you explain what happened in 2-3 sentences? 
Strings are like clauses in a sentence. When you add strings together using the paste function, you take 2 clauses separated by a comma and turn them into a sentence. It creates a complete thought or complete action rather than separate actions that don't make much sense alone. 
> 
...
>Line 98: The code asks the computer if 5 is equal to 10. Because 5 is not equal to 10, the terminal states that it is false. Line 99: The code asks the computer if 3 is less than 4. Because 3 is less than 4, the terminal states that it is true. Line 100 and 101 ask if 5 is unequal to 10 and 5. Since 5 is not equal to 10, line 100 is false. Since 5 is equal to 5, line 101 is true. Line 102 asks if 3 is equal to 4 OR 3. Since 3 is equal to 3, the line is true. Line 103 asks if 3 is not equal to 4 and (&) not equal to 5. Since 3 is not equal to either number, the line is true. If 3 was equal to either number, it would be false, since the ampersand specifies that 3 must be unequal to both numbers.
...
helios.age <- 7
illum.age <- 5
helios.age * illum.age
```
35

(@) Try creating two variables that store strings. Can you concatenate those two variables together? 
```{r}
x <- "Wednesday's Schedule"
y <- "Hist 1730 Hist 8500 Hist 8520"
paste(x, y)
```
Wednesday's Schedule Hist 1730 Hist 8500 Hist 8520
### Vectors
...
>The vector "my.numbers" contains the numbers 2, 4, 6, and 8. By multiplying the vector "my.numbers" by 2, you multiply each variable by 2 individually. In R terminal, you get the output 4, 8, 12, and 16. 
...
```
(@) Explain in a few sentences, what happened in the code above? 
The code above isolates the third number in the vector "my.numbers", which is 6. Then it multiplies 6 by 2. 
>

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
mygrocerylist <- c("milk", "bread", "eggs", "ice cream", "potato chips")
...
the floor function takes a numeric argument coded as x or a variable and returns a vector containing any number less than the argument contained in the variable.
I assume that means if x is something like y+6<20 it will yield a vector containing all numbers that could apply to the y value. 
>
...
>In row z, the value from x (a number from 1-5) is multiplied by 2. So 1x2 is 2, 2x2 is 4, 3x2 is 6, 4x2 is 8, and 5x2 is 10. Then, the y value, 1, is added to (x*2). 2+1 is 3, 4+1 is 5, 6+1 is 7, 8+1 is 9, and 10+1 is 11. 
...
The documentation gives a description of the dataset that states where the data was collected from Bob Damron's guidebooks, published 1965-1985. It has a last updated date of December 2022. It offers a usage that is just "gayguides," a formatting description, and two examples of codes you can run using the dataset.
>The data
...
I tried several things using the paste and command prompts we talked about during tech table last week. All of those options were overcomplicating things-I went onto stack overflow to look for a forum that dealt with head and tail commands. All I had to do was add "10" to the command to run the code. 
>
...
>gayguides is the dataset the code is pulling from. The $ says that I'm requesting a specific chunk of the dataset. Title specifies which column to pull from. 500 tells the code which row to pull out and display.
...
min(almshouse_admissions$Year.Admitted)
max(almshouse_admissions$YearDischarged)
min(almshouse_admissions$Year.Admitted)
almshouse_admissions[which.min(almshouse_admissions$date)]
```

(@) What did you learn about the dataset? What can you tell me about it? 
>Using the string command I learned that the database contains the name, the year and month admitted, the gender, a clerk's description of the person, and the information about their discharge from the Pennsylvania almshouse. There are 1400 rows of data in the dataset. The dataset is organized alphabetically by last name. The final year someone was discharged in this dataset was 1813. When running code to see what the minimum year was, I kept getting "0" which I'm not sure how to fix-it worked above. Using statology, I tried a different method (shown above) and still didn't get a solution. 
