...
sotu.length2 <- tidy_sotu %>% separate(years_active, into = c("year_start", "year_end"), sep = "-", convert = TRUE) %>% group_by(president) %>% summarize(term.length = year_end - year_start) %>% filter(term.length == 4) 
