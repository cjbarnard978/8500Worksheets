...
BWVfiltered_combined <- BWVfiltered %>%
  ...
  BWVcoords <- BWVfiltered %>% geocode(addr = street.address, method = "osm", lat = latitude, long = longitude)
  head(BWVcoords)
...
ggplot() + geom_map( data = SCbasemap, map = SCbasemap, aes(long, lat, map_id=region), fill = "lightgray", color = "black") + geom_point(data = gayguidessc, mapping = aes(x = lon, y = lat), color = "red")
